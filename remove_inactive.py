...
def time_diff(last_contact_time): # inputs a date of format Year-Month-Date e.g. 2022-07-13
    if last_contact_time != None:
        last_time = last_contact_time[:10].split('-') # splits this string into a list around the '-'
...
def inactive():
    ...
    with open("C:/Users/jbackon/OneDrive - Babson College/Security/CB Removal Logs/removed_inactive_" + str(date.today()) + '.txt', 'w') as f: # creates a new log file
        for device in query: # loops through all devices in the CB database
            if device.policy_name == 'Standard': # ensures that only devices in the standard policy will be checked
                check = time_diff(device.last_contact_time) # checks the last time the device checked into the CB console
                if check >= 30: # if the check was more than 30 days ago
